...
export async function fetchProductos(params: FetchProductosParams = {}): Promise<StrapiResponse<ProductoStrapi>> {
  ...
  const response = await fetch(`${STRAPI_URL}/api/productos?${searchParams.toString()}`);
...
export async function fetchProductoBySlug(slug: string): Promise<ProductoStrapi | null> {
