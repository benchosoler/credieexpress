...
- [x] 2.6 Retrofit `FullFeature.tsx`: same treatment via `MagazineCard`'s `variant="feature"` + `descriptionMode="full"`; keeps rendering full `descripcion` regardless of `descripcionCorta` — *fullFeature Ignores descripcionCorta*. Two decorative-only elements were dropped in the consolidation (not spec requirements): the `.featured-title-line` gradient bar under the title, and the SVG truck icon beside "Envio incluido"; the custom 32x32 empty-state icon was replaced by the same plain-text empty label every other card uses. Documented as a deviation below.
